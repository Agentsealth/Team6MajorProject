...
public class SaveLoadMenuTest : MonoBehaviour
{
    ...
    {
        saveName = autoSaveSlot1.GetComponentInChildren<Text>();
        bool exist = SaveLoad.CheckFileExist(saveName.ToString());
        if(exist == true)
        {
            saveGameName = autoSaveSlot1.GetComponentInChildren<Text>().ToString();
            GameData loadedGame = SaveLoad.Load(saveGameName);
            dayProgress.text = "Day: " + loadedGame.dayNumber.ToString();
            dateTime.text = File.GetCreationTime(saveGameName).ToString();
        }
        else if(exist == false)
        {
            saveGameName = autoSaveSlot1.GetComponentInChildren<Text>().ToString();
